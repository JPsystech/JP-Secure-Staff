...
interface TopbarProps {
  user?: { full_name?: string; role_name?: string; role?: string; department?: { name?: string } } | null
  /** Page title shown in header (optional) */
  title?: string
  /** Breadcrumbs: array of { label, href? }. Last item is current page. */
...
export function Topbar({ user, title, breadcrumbs, leftContent, className }: TopbarProps) {
    ...
    >
      <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-4">
        ...
        {breadcrumbs && breadcrumbs.length > 0 ? (
          ...
          <span className="text-lg font-semibold text-foreground">JP Secure Staff</span>
        )}
      </div>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuLabel>
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">{user.role_name || user.role}</p>
